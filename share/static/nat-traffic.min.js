...
128,192,256,512];c=[];for(d in a)if(d=a[d],c.push([1024*d,d]),1024*d>b.datamax)break;return c}}};d={};f=function(b,a,c){b="^.*$";location.hash.length&&"#!/"==location.hash.substr(0,3)&&(b=location.hash.substr(3),b=-1==b.indexOf("/")?"^"+b:"^"+b.split("/")[1]+"$");b=new RegExp(b);a=[];for(c in d["in"])c.match(b)&&a.push(d["in"][c]);e.plot(".plug-flot",a,h)};g=function(){e.getJSON("v/traffic.json",function(b,a,c){d["in"]={};d.out={};for(a in b)for(c in d["in"][a]={label:a,data:[]},d.out[a]={label:a,
data:[]},b[a])d["in"][a].data.push([1E3*c,b[a][c]["in"]/60]),d.out[a].data.push([1E3*c,b[a][c].out/60]);f()})};e(window).on("hashchange",f);setInterval(g,6E4);g()})(jQuery);
